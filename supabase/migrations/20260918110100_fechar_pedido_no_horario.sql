-- =============================================================================
-- fechar_pedido passa a respeitar o horario de funcionamento.
--
-- A unica mudanca e a condicao que recusa pedido com a loja fechada: antes
-- olhava so `is_open`, agora exige que a chave da mao E o horario concordem.
-- O resto do corpo continua o mesmo — preco recalculado do cardapio, cupom,
-- comissao, estoque, mesa.
--
-- Por que reescrever a funcao inteira em vez de remendar: ela e o unico caminho
-- pelo qual um cliente cria pedido, e uma funcao assim vive no arquivo que a
-- define por completo. Remendo em plpgsql e `create or replace` de qualquer
-- jeito; melhor que o arquivo conte a verdade inteira.
-- =============================================================================
