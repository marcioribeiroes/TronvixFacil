...
export default async function PaginaDoPedido({ params }: PageProps<"/pedidos/[id]">) {
  ...
  const { data: pedido } = await supabase
    .from("orders")
    .select(
      "id, number, status, fulfillment, customer_name, address_summary, address_district, notes, subtotal_cents, delivery_fee_cents, discount_cents, total_cents, coupon_code, created_at, restaurants(name), order_items(id, product_name, quantity, total_cents, order_item_addons(addon_name)), payments(method, timing, change_for_cents), deliveries(status)",
    )
    .eq("id", id)
    .maybeSingle()
